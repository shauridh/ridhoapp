            ...
            {rows.map(({ ingredient, perDay, projection }) => (
              <tr key={ingredient.id} className="border-b border-hairline text-ink">
                ...
                <td className="px-3 py-2 text-right">
                  {perDay.toLocaleString("id-ID", { maximumFractionDigits: 2 })} {ingredient.unit}
                </td>
                <td className="px-3 py-2 text-right">
                  {ingredient.stock_qty.toLocaleString("id-ID")} {ingredient.unit}
                </td>
                <td className="px-3 py-2 text-right">
                  {projection.neededQty.toLocaleString("id-ID", { maximumFractionDigits: 2 })}{" "}
                  {ingredient.unit}
                </td>
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={5} className="px-3 py-4 text-center text-ink-soft">
                  Belum ada bahan baku untuk diproyeksikan.
                </td>
              </tr>
            )}
          </tbody>
