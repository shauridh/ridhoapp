...
export async function deleteProduct(id: string) {
  ...
  if ((orderCount ?? 0) > 0) {
    return {
      ok: false as const,
      error: `Produk tidak bisa dihapus karena sudah masuk ${orderCount} transaksi.`,
    };
  }
  ...
  const { count: comboCount } = await supabase
    .from("combo_items")
    .select("id", { count: "exact", head: true })
    .or(`combo_product_id.eq.${id},child_product_id.eq.${id}`);

  if ((comboCount ?? 0) > 0) {
    return {
      ok: false as const,
      error: `Produk tidak bisa dihapus karena masih dipakai paket/combo.`,
    };
  }
