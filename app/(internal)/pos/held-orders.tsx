...
export function HeldOrders({ refreshKey, onResume }: Props) {
  ...
  const handleDelete = async (o: HeldOrder) => {
    const ok = await dialog.confirm(`Hapus pesanan tersimpan "${o.label}"?`, "Hapus Pesanan");
    if (!ok) return;
  ...
  if (orders.length === 0) {
    return <p className="py-8 text-center text-sm text-ink-soft">Belum ada pesanan tersimpan.</p>;
  }
