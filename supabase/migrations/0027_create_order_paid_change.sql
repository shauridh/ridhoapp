-- ============================================================
-- 0027: Update create_order to store paid and change for cash
-- Fix: gunakan struktur RPC dari 0024 (ingredients, stock_movements, _active_recipe_id)
-- ============================================================

DROP FUNCTION IF EXISTS public.create_order(numeric, text, jsonb, numeric, text, text);
DROP FUNCTION IF EXISTS public.create_order(numeric, text, jsonb, numeric, text, text, numeric, numeric);
...
BEGIN
  ...
  INSERT INTO public.orders
    (shift_id, total, payment_method, source, status, created_by, order_number,
     discount_amount, discount_type, table_number, paid, change)
  VALUES
    (v_shift, p_total, p_payment_method, 'cashier', 'completed', v_uid, v_next_order_num,
     COALESCE(p_discount_amount, 0), p_discount_type, p_table_number, p_paid, p_change)
  ...
  LOOP
    ...
    -- Kurangi stok menggunakan helper yang sudah ada (_active_recipe_id)
...
GRANT EXECUTE ON FUNCTION public.create_order(numeric, text, jsonb, numeric, text, text, numeric, numeric) TO authenticated;
