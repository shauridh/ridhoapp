-- ============================================================
-- 0031: Update close_shift - routing pembayaran ke akun yang tepat
-- Owner withdrawal  → akun is_owner = true (akun Ridho)
-- QRIS             → akun tipe = 'bank' pertama yang aktif
-- ============================================================
...
BEGIN
  ...
  SELECT id INTO v_bank_akun_id
  FROM public.akun
  WHERE tipe = 'bank' AND aktif = true
  ORDER BY created_at ASC
  LIMIT 1;

  -- Catat total omzet ke cashflow bisnis (tanpa akun_id, ini omzet keseluruhan)
  ...
  IF v_income > 0 THEN
    ...
    VALUES
      (v_today, 'in', v_income, 'income', 'sale',
       p_shift_id, 'Penjualan shift ' || p_shift_id, v_uid);
  END IF;

  -- QRIS masuk ke akun Bank
  IF v_qris > 0 AND v_bank_akun_id IS NOT NULL THEN
    INSERT INTO public.cashflow_entries
      (entry_date, direction, amount, kind, source, ref_id, note, created_by, akun_id)
    VALUES
      (v_today, 'in', v_qris, 'income', 'sale',
       p_shift_id, 'QRIS shift ' || p_shift_id, v_uid, v_bank_akun_id);
  END IF;

  -- Transfer masuk ke akun Bank juga (jika ada)
  IF v_transfer > 0 AND v_bank_akun_id IS NOT NULL THEN
    INSERT INTO public.cashflow_entries
      (entry_date, direction, amount, kind, source, ref_id, note, created_by, akun_id)
    VALUES
      (v_today, 'in', v_transfer, 'income', 'sale',
       p_shift_id, 'Transfer shift ' || p_shift_id, v_uid, v_bank_akun_id);
  END IF;

  -- Owner withdrawal: keluar dari bisnis, masuk ke akun owner
  IF p_owner_withdrawal > 0 THEN
    -- OUT dari bisnis
    INSERT INTO public.cashflow_entries
      (entry_date, direction, amount, kind, source, ref_id, note, created_by)
    VALUES
      (v_today, 'out', p_owner_withdrawal, 'withdrawal', 'drawer',
       p_shift_id, 'Ambil owner shift ' || p_shift_id, v_uid);
    ...
    IF v_owner_akun_id IS NOT NULL THEN
      INSERT INTO public.cashflow_entries
        (entry_date, direction, amount, kind, source, ref_id, note, created_by, akun_id)
      VALUES
        (v_today, 'in', p_owner_withdrawal, 'withdrawal', 'drawer',
